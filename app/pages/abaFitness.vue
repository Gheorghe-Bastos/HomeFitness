...
<template>
  <main class="flex justify-around items-center w-full min-h-[90%]">
    <div class="flex flex-col lg:flex-row m-4 sm:m-8 h-full w-full justify-between gap-4">
      <UPageCard
        class="w-full lg:w-sm h-full p-4 light:shadow-[0px_0px_10px]"
        :ui="{
      ...
      <div
        v-if="carregandoAbaFitness"
        class="flex flex-col justify-around gap-7 h-full w-full m-0"
      >
      ...
      <div
        v-else-if="tabelaExiste"
        ip="dadosDiv"
        class="flex flex-col justify-around gap-7 h-full w-full m-0"
      >
        <div class="flex flex-col sm:flex-row justify-around h-full gap-5">
          <div
            v-for="(item, index) in listaDadosTBMIMC"
            :key="index"
            class="flex w-full h-full"
          >
            <UPageCard
              class="w-full h-full light:shadow-[0px_0px_10px]"
              spotlight
              spotlight-color="primary"
            >
        ...
        <div class="grid grid-cols-2 sm:flex sm:justify-around h-full gap-3">
          ...
          >
            ...
            <p class="mt-3 text-3xl font-semibold text-primary">
              {{ usuarioAutenticado.perfil?.GET }} kcal
            </p>
            <p class="mt-2 text-sm text-neutral-500">
              Este é o seu gasto energético diário levando em conta sua atividade
              física.
            </p>
          </UPageCard>

          <div
            v-for="(itens, index) in listaDadosMacros"
            :key="index"
            class="w-full h-full"
          >
            <UPageCard
              class="flex h-full light:shadow-[0px_0px_10px]"
              spotlight
              spotlight-color="primary"
            >
              <p class="text-sm font-medium uppercase tracking-[0.2em] text-neutral-500">
              ...
              <p class="mt-3 text-3xl font-semibold text-primary">
                {{ itens.valor }} {{ itens.unidade }}
              </p>
              <p class="mt-2 text-sm text-neutral-500">
                Quantidade diária estimada para a sua meta desejada.
              </p>
            </UPageCard>
          </div>
        </div>
      ...
      <UPageCard
        v-else
        class="w-full h-full font-bold p-4 light:shadow-[0px_0px_10px]"
        :ui="{
